---
title: "amel_bombus_queen_comparison"
...
```
```{r library_setup, echo = FALSE, message = FALSE, warning = FALSE}
#renv::snapshot()
```
...
# <b>Questions:</b>
  1. Are there differences in the usage of nAChR subunits in the queen brains of A. mellifera and B. terrestris?
Datasets used:
<br>-Liberti et al. 2015 (amel)
<br>-Manfredini et al. 2015 (amel)
<br>-Manfredini et al. 2018 (bter)
<br>    
...
sample_amel <- dir(file.path("kallisto_results_trimmed/species_amel"))

# Read metadata table
samples_amel <- read.csv(
  file.path("kallisto_results_trimmed/metadata", "species_amel.csv"),
     header = TRUE,
       stringsAsFactors = TRUE)

# Read .h5 files in
filenames_amel <- file.path(
  "kallisto_results_trimmed/species_amel",
  samples_amel$sample,
  "abundance.h5")
...
############
# Set sample identifiers using the names of the kallisto output directories
sample_bter <- dir(file.path("kallisto_results_trimmed/species_bter"))

# Read metadata table
samples_bter <- read.csv(
  file.path("kallisto_results_trimmed/metadata", "species_bter.csv"),
     header = TRUE,
       stringsAsFactors = TRUE)

# Read .h5 files in
filenames_bter <- file.path(
  "kallisto_results_trimmed/species_bter",
  samples_bter$sample,
  "abundance.h5")
...
# Extract single-study samples (Manfredini)
gene_list_amel <- gene_list_amel[, -c(5:7)]
# Select four random samples for Bombus 
gene_list_bter <- gene_list_bter[, c(2,3,5,6,7)]
```
...
ratios_transf_amel <- data.frame(ratios_transf_amel[1], stack(ratios_transf_amel[2:5]))
# Add species column
...
ratios_transf_amel <- ratios_transf_amel %>% mutate_all(~gsub("abundance.|_j", "", .)) %>% mutate_all(~gsub("queen", "apis", .))
# Make TPMs numeric
...
ratios_transf_bter <- data.frame(ratios_transf_bter[1], stack(ratios_transf_bter[2:5]))
# Add species column
...
ggplot2::ggplot(species) + geom_abline(colour = "grey", linetype = "dashed") + 
  ...
  geom_point(size = 1.5, aes(amel_tpm, bter_tpm, colour=nAChRs...1)) + geom_smooth(colour = "black", size = 0.7, method="lm", se = FALSE, aes(bter_tpm, amel_tpm)) + ggtitle("Queens") +
