...
samples_table <- read.csv(
  file.path(
    ...
    "bterrestris_colgan_metadata.csv"
  ),
  header = TRUE,
...
dds$caste <- relevel(dds$caste, ref = "queen")
...
lfc_nachrs <- lfc %>%
  ...
  dplyr::mutate(contrast = "WorkerQueen") %>%
  dplyr::mutate_if(is.numeric, round, digits = 4)
